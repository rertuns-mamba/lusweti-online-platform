    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div
            ...
            @foreach ($pages as $page)
                ...
                <a href="{{ $url }}" aria-current="{{ $isActive ? 'page' : 'false' }}"
                    class="relative group shrink-0 py-2 outline-none focus-visible:text-red-600 transition-colors duration-200 {{ $isActive ? 'text-red-600' : 'hover:text-red-600' }}">

                    {{ $page->title }}

                    <span
                        class="absolute bottom-0 left-0 w-full h-[3px] bg-red-600 transform origin-left transition-transform duration-300 ease-out {{ $isActive ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100 group-focus-visible:scale-x-100' }}"></span>
                </a>
            @endforeach
        </div>
    </div>
