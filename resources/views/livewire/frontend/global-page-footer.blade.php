                    ...
                    <ul class="grid grid-cols-2 gap-y-4 gap-x-6 text-[10px] font-bold uppercase tracking-widest">
                        ...
                        <li>
                            <a href="{{ route('page.show', $page['slug']) }}"
                                class="relative inline-block pb-1 group transition-colors duration-200 {{ request()->is($page['slug']) ? 'text-red-500' : 'text-gray-300 hover:text-white' }}">
                                {{ $page['title'] }}

                                <!-- Smooth animated underline adapted for dark footer surface -->
                                <span class="absolute bottom-0 left-0 w-full h-[2.5px] bg-red-600 transform origin-left transition-transform duration-200 {{ request()->is($page['slug']) ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100' }}"></span>
                            </a>
                        </li>
                        @empty
