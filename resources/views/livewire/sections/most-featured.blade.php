<div class="max-w-screen-2xl mx-auto  py-8"
    ...
    <div class="mb-8 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            <button @click="scrollPrev()" class="p-2 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-900 transition-colors focus:outline-none" aria-label="Scroll left">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                </svg>
            </button>
            <button @click="scrollNext()" class="p-2 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-900 transition-colors focus:outline-none" aria-label="Scroll right">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div
        ...
        <article
            ...
            @if($isVideo) x-init="initVideoObserver()" @endif
            ...
            <a href="{{ $targetUrl }}"
                ...
                {{-- Play Icon Overlay --}}
                <div
                    :class="playing ? 'opacity-0 scale-95' : 'opacity-100 scale-100'"
                ...
                {{-- Duration Badge --}}
                <div class="absolute bottom-2 right-2 bg-black/75 backdrop-blur-sm text-white text-xs font-semibold px-2 py-1 rounded z-20">
                    {{ $item->duration ?? '0:00' }}
                </div>
                @endif
            </a>
            ...
            <div class="p-5 flex flex-col flex-grow select-none">
                <span class="text-xs font-bold {{ $isVideo ? 'text-red-600' : 'text-blue-600' }} uppercase tracking-wider mb-2">
